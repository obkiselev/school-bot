...
- [x] Таблица лидеров по XP среди учеников
- [x] Еженедельные челленджи
- [x] Достижения за регулярность (30 дней и т.д.)
- [x] Обмен результатами между учениками
- [x] Новые языки (французский, немецкий)
- [x] Квизы по школьным предметам (математика, история, биология)
- [x] Новые типы вопросов (сопоставление, аудио)
- [x] Импорт вопросов из файла (учитель может загрузить)
- [x] Голосовые ответы в квизах (Whisper/STT)

## Завершено в v1.5.1

- [x] Команда `/social_admin` для админа (сводка: XP-лидеры, недельный челлендж, регулярность)

## Завершено в v1.5.2

- [x] Исправлен `/help` (экранирование `/share &lt;token&gt;` для `parse_mode=HTML`)
- [x] Добавлен тест `tests/test_start_help.py` на корректный help-рендеринг

## Завершено в v1.5.3

- [x] Добавлен `work/deploy-school-bot.ps1` в репозиторий
- [x] Исправлены ложные падения deploy (ожидание `active`, корректная передача remote-script, совместимый `systemctl status`)

## Завершено в v1.6.0

- [x] Встроенная веб-панель админа `/admin` (aiohttp): карточки метрик и график тестов.
- [x] Групповые рассылки из веб-панели с фильтром ролей и режимом dry-run.
- [x] Журнал рассылок в БД (`admin_broadcasts`, `admin_broadcast_recipients`).
- [x] Миграция `010_admin_panel.sql` + автоприменение в `core/database.py`.
...
### v1.4.0 — Стабилизация LLM-моста и эксплуатационная диагностика

**LLM/Bridge (`services/health_check.py`, `llm/client.py`, `handlers/quiz_settings.py`):**
- Для схемы `bridge + tunnel` в `/health` недоступный `direct localhost` теперь помечается как ожидаемый резервный путь, если `bridge` уже доступен.
- В тестировании явно выводится режим генерации: `Режим: LLM` или `Режим: fallback`.
- Для `fallback` показывается техническая причина переключения (например, ошибка direct endpoint).

**Тесты и надежность (`tests/test_health_check.py`, `tests/test_llm_client.py`, `tests/test_test_generator_fallback.py`):**
- Добавлены/актуализированы тесты для health-логики bridge/direct и fallback-сценариев.
- Подтверждена стабильность полного тестового набора проекта.

**Эксплуатация и документация (`README.md`, `readme.txt`, `PROGRESS.md`):**
- Обновлены инструкции по работе в архитектуре: LLM на локальном ПК, сервер через bridge/tunnel.
- Синхронизирована историчность разработки и версия релиза до `1.4.0`.
