...
- [ ] Определить способ защищённого соединения server -> local LLM (без открытия небезопасного публичного доступа)
- [ ] Добавить конфиг и режим подключения к удалённому LLM-хосту вместо `localhost`
- [ ] Продумать fallback для квизов при недоступности локального LLM
