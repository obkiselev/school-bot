...
## Статус: Напоминания и планировщик

---

## Ближайший план (v1.2.1)

- [ ] Спроектировать схему работы: бот на VPS + LLM на локальном ПК
- [x] Определить способ защищённого соединения server -> local LLM (без открытия небезопасного публичного доступа)
- [x] Добавить конфиг и режим подключения к удалённому LLM-хосту вместо `localhost`
- [x] Продумать fallback для квизов при недоступности локального LLM
- [ ] Зафиксировать эксплуатационные инструкции для запуска гибридной схемы
