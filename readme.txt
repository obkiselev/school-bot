...
v1.2.1 — LLM-мост: бот на сервере + локальная LLM
  - Спроектировать безопасный канал между сервером и локальным LLM-хостом
  - Убрать жёсткую привязку LLM к localhost сервера
  - Сохранить работоспособность тестов/квизов без деплоя LLM на VPS
  - Подготовить режимы fallback при недоступности локального LLM
